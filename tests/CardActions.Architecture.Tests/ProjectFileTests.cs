    ...
    public ProjectFileTests(ITestOutputHelper output)
    {
        ...
        if (File.Exists(_directoryBuildPropsPath))
            ...
            throw new FileNotFoundException(
                $"Nie znaleziono pliku Directory.Build.props pod ścieżką: {_directoryBuildPropsPath}");
    ...
    public void ProjectFiles_ShouldNotContainDuplicateProperties()
    {
        ...
        foreach (var projectFile in _projectFiles)
        {
            ...
            if (duplicateProperties.Any())
                errors.Add(
                    $"Plik {Path.GetFileName(projectFile)} zawiera zduplikowane właściwości: {string.Join(", ", duplicateProperties)}");
        }
        ...
        foreach (var projectFile in _projectFiles)
        {
            ...
            foreach (var packageReference in packageReferences)
            {
                ...
                if (versionAttribute != null)
                    errors.Add(
                        $"Plik {Path.GetFileName(projectFile)} zawiera atrybut Version dla pakietu {packageReference.Attribute("Include")?.Value}");
            }
        }
