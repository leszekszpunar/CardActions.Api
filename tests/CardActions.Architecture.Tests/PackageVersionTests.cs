...
public class PackageVersionTests
{
    ...
    public PackageVersionTests(ITestOutputHelper output)
    {
        ...
        if (File.Exists(_directoryPackagesPropsPath))
            ...
            throw new FileNotFoundException(
                $"Nie znaleziono pliku Directory.Packages.props pod ścieżką: {_directoryPackagesPropsPath}");
    ...
    private string? FindSolutionDirectory(string startPath)
    {
        ...
        {
            if (File.Exists(Path.Combine(currentDir, "CardActions.sln"))) return currentDir;
        ...
        foreach (var projectFile in _projectFiles)
        {
            ...
            foreach (var packageRef in packageReferences)
            {
                ...
                if (versionAttribute != null && !string.IsNullOrEmpty(versionAttribute.Value) &&
                    ...
                    errors.Add(
                        $"Project {Path.GetFileName(projectFile)} specifies version {versionAttribute.Value} for package {packageRef.Attribute("Include")?.Value}");
            }
        }
    ...
    public void AllPackages_ShouldHaveVersionsSpecified()
    {
        ...
        if (packageVersions == null || !packageVersions.Any())
            ...
            foreach (var package in packageVersions)
            {
                ...
                if (string.IsNullOrEmpty(packageVersion)) errors.Add($"Pakiet {packageName} nie ma określonej wersji");
            }
